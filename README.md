...
- `sinol-make outgen` -- Generate output files using the model solutions.
Output files which weren't generated by sinol-make (for example hand-written example outputs) are never overwritten.
Instead, they are verified against the model solution's output -- with the checker, if the package has one -- and
outgen fails if any of them is wrong. Use the `--overwrite` flag to replace them with the model solution's outputs.
Run `sinol-make outgen --help` to see available flags.
