...
def test_get_ingen():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ingen_path = get_ingen_path("prog/abcingen.cpp")
        assert os.path.basename(ingen_path) == "abcingen.cpp"

        with pytest.raises(SystemExit) as e:
            get_ingen_path("prog/abcingen.c")
        ...
        with pytest.raises(SystemExit) as e:
            get_ingen_path("prog/geningen.cpp")
        assert e.type == SystemExit
        assert e.value.code == 1
...
def test_get_correct_solution(create_package):
    ...
    """
    correct_solution_path = package_util.get_correct_solution().path
...
def test_generate_output(create_package):
    ...
    correct_solution = package_util.get_correct_solution().path
